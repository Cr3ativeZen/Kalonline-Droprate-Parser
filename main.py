...
def homepage(variable):
    ...
    monsters = list(select)
    query = "SELECT i.NAME,ROUND(igg.GroupDropChance/1000*gi.ItemDropChance/1000 * mig.Rolls,7) AS dropchance,igg.IDItemGroup FROM Monsters_ItemGroups mig " \
    ...
    return render_template('monster.html', p=p, f=f,monsters=monsters,ret = result,mobname = mobname)
